...
public class AuditServiceImp implements AuditService {
    ...
    @Override
    public void updateAudit(AuditModel updateAudit) {

        Optional<AuditModel> auditOptional = auditRepository.findById(updateAudit.getId());
        if (auditOptional.isPresent()) {
            AuditModel audit = auditOptional.get();
            auditMapper.toEntity(updateAudit);
            auditRepository.save(audit);
        }
    }
