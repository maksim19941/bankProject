...
import com.bank.antifraud.controller.ControllerAccountTransfers;
...
public class ControllerAccountTransfersTest {
    ...
    void setUp() {
        ...
        accountTransfer = AccountTransfersDTO.builder()
                .id(1L)                          // Set ID
                .account_transfer_id(12345L)         // Set Account transfer ID
    ...
    void getAllAccountTest() throws Exception {
        when(accountService.getListSAccountTransfers()).thenReturn(Arrays.asList(accountTransfer));
    ...
    void deleteAccountTest() throws Exception {
        ...
        mockMvc.perform(delete("/account_transfers/{id}", 1))
                .andExpect(status().isOk());
        Mockito.verify(accountService).deleteAccount(eq(1L));
    ...
    void getAccountTestException() throws Exception {
        when(accountService.getAccountTransfer(anyLong())).thenReturn(null);

        mockMvc.perform(get("/account_transfers/{id}", 999))
    ...
    void deleteAccountExceptionIsNotFound() throws Exception {
        ...
        doThrow(new EntityNotFoundException("An object with this ID was not found for deletion, ID: " + id,
                "accountTransferServiceImpl.deleteAccountTransfer")).when(accountService).deleteAccount(anyLong());
    ...
    void saveAccountExceptionTestValidationException() throws Exception {
        ...
        doThrow(new ValidationException("Account transfer ID must not be null.",
                "AccountTransferServiceImpl.saveAccount", HttpStatus.BAD_REQUEST))
                .when(accountService)
                .saveAccount(any(AccountTransfersDTO.class));
