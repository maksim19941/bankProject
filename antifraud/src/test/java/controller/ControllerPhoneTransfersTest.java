...
import com.bank.antifraud.controller.ControllerPhoneTransfers;
...
public class ControllerPhoneTransfersTest {
    ...
    void setUp() {
        ...
        phoneTransfer = PhoneTransfersDTO.builder()
                .id(1L)                          // Set ID
                .phone_transfer_id(12345L)         // Set Phone transfer ID
    ...
    void getAllPhoneTest() throws Exception {
        when(phoneService.getListPhoneTransfers()).thenReturn(Arrays.asList(phoneTransfer));
    ...
    void savePhoneTest() throws Exception {
        ...
        verify(phoneService, times(1)).savePhone(any(PhoneTransfersDTO.class));
    }
    ...
    void getPhoneTestException() throws Exception {
        when(phoneService.getPhoneTransfer(anyLong())).thenReturn(null);

        mockMvc.perform(get("/phone_transfers/{id}", 999))
    ...
    void deletePhoneExceptionIsNotFound() throws Exception {
        ...
        doThrow(new EntityNotFoundException("An object with this ID was not found for deletion, ID: " + id,
                "phoneTransferServiceImpl.deletePhoneTransfer")).when(phoneService).delete(anyLong());
    ...
    void savePhoneExceptionTestValidationException() throws Exception {
        ...
        doThrow(new ValidationException("Phone transfer ID must not be null.",
                "PhoneTransferServiceImpl.savePhone", HttpStatus.BAD_REQUEST))
                .when(phoneService)
                .savePhone(any(PhoneTransfersDTO.class));
