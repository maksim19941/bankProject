...
public class AuditAspect {
    ...
    @AfterReturning(
            pointcut = "execution(* com.bank.authorization.service.*.save*(..))",
            returning = "user")
    public void auditSave(JoinPoint joinPoint, User user) throws JsonProcessingException {
        ...
        audit.setCreatedBy(objectMapper.readTree(json).get("profile_id").asText());
    ...
    @AfterReturning(
            pointcut = "execution(* com.bank.authorization.service.*.update*(..))",
            returning = "user")
    public void auditUpdate(JoinPoint joinPoint, User user) throws JsonProcessingException {
        MethodSignature signature = (MethodSignature) joinPoint.getSignature();
        String json = objectMapper.writeValueAsString(user);
        ...
        if (oldAudit.isPresent()) {
            ...
            audit.setModifiedBy(objectMapper.readTree(json).get("profile_id").asText());
